...
import { PUBLIC_SIRIUS_LOG_CHANNEL_ID, PUBLIC_SIRIUS_GUILD_ID } from '$env/static/public';
...
export async function sendDenyLog(user: DiscordUser, application: Application) {
  ...
  await makeDiscordAPIRequest(
    ...
    "embeds": [
      {
        "title": "<:Sirius:1124023877098553365>  Application Denied",
  ...
  await makeDiscordAPIRequest(
    `channels/${PUBLIC_SIRIUS_LOG_CHANNEL_ID}/messages`,
    'POST',
    `{
    "embeds": [
      {
